...
export const extractExports = (content) => {
  ...
  // for "X as Y" keep the public name (Y); internal binding names are
  // implementation details that change without affecting the API surface
  const addNames = (rawNames, prefix) => {
    for (const raw of rawNames.split(',')) {
      const value = raw.trim().replace(/^.*\s+as\s+/, '');
      if (value) exported.add(`${prefix}:${value}`);
    }
  };
  ...
  for (const line of lines) {
    ...
    const typ = line.match(/^export type\s+([A-Za-z0-9_]+)\b/);
    if (typ) exported.add(`type:${typ[1]}`);

    const cnst = line.match(/^export declare const\s+([A-Za-z0-9_]+)/);
    if (cnst) exported.add(`const:${cnst[1]}`);

    const reexportType = line.match(/^export type \{([^}]+)\};?$/);
    const reexport = reexportType ? null : line.match(/^export \{([^}]+)\};?$/);
    if (reexportType) addNames(reexportType[1], 'reexport-type');
    if (reexport) addNames(reexport[1], 'reexport');
  }
