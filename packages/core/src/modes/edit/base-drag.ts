...
export abstract class BaseDrag extends BaseEdit {
      ...
      if (event.originalEvent.ctrlKey) {
        ...
        if (!currentSelection.has(this.featureData.id)) {
          this.gm.features.setSelection(
            [...currentSelection, this.featureData.id],
            true,
          );
        }
      } else {
  ...
  isPointBasedShape(): boolean {
    ...
    return (
      !!this.featureData &&
      this.pointBasedShapes.includes(this.featureData.shape)
    );
  }

  abstract handleGmEdit(
    event: GmSystemEvent,
  ): Promise<MapHandlerReturnData> | MapHandlerReturnData;

  async alignShapeCenterWithControlMarker(
    featureData: FeatureData,
    event: BaseMapEvent,
  ) {
  ...
  async moveFeature(featureData: FeatureData, lngLatDiff: LngLatDiff) {
    ...
    const customShapeUpdateHandlerFunc =
      this.gm.options.settings.customShapeUpdateHandler;
    let updatedGeoJson:
    ...
    if (!updatedGeoJson) {
      const shapeUpdateMethod =
        this.getUpdatedGeoJsonHandlers[featureData.shape];
    ...
    if (
      !isEqual(featureData.getGeoJson().properties, updatedGeoJson.properties)
    ) {
  ...
  async moveCircle(
    ...
    if (!Array.isArray(shapeCenter)) {
      log.error(
        "BaseDrag.moveCircle: missing center in the featureData",
        featureData,
      );
      return null;
    }
