  ...
  test('awaitDataUpdatesOnEvents=true should have feature in source when gm:create fires', async ({
    ...
    await page.evaluate(
      (context) => {
        ...
        window.geoman.mapAdapter.once('gm:create' as AnyEventName, (event: { feature: { id: string; source: { getGeoJson: () => { features: Array<{ id?: string | number }> } } } }) => {
          const featureId = event.feature.id;
          const sourceGeoJson = event.feature.source.getGeoJson();
          const featureInSource = sourceGeoJson.features.some((f) => f.id === featureId);
